...
class ChatGPT:
    ...
    async def append_history(self, user_id: str, text: str) -> str:
        self.memory.append('SAM', {'role': '### username', 'content': text})
        return

    async def prime(self, user_id: str, records: list) -> str:
        output = [self.memory.append('SAM', dict_) for dict_ in records]
        return
